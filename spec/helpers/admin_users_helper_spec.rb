...
RSpec.describe AdminUsersHelper do
  ...
  let(:scope_instance) { instance_double(AdminUserViewPolicy::Scope) }

  before do
    allow(AdminUserViewPolicy::Scope).to receive(:new).with(current_user, nil).and_return(scope_instance)
    user = current_user
