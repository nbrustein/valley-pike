...
RSpec.describe "UsersMutate", type: :request do
  ...
  describe "GET /admin/users/new" do
    context "when the current user is allowed to create users" do
      it "renders the header" do
        assert_success { act(path: new_admin_user_path) }
  ...
  describe "POST /admin/users" do
    context "when the current user is allowed to create users" do
      ...
      it "Calls CreateUser and redirects to admin_users_path" do
        assert_redirect(to: admin_users_path) {
          act(path: admin_users_path, params: valid_create_params)
        }
      end
    end
  ...
  describe "GET /admin/users/:id/edit" do
    ...
    context "when the current user is allowed to update the target user" do
      it "renders the form" do
        assert_success { act(path: edit_admin_user_path(id: target_user.id)) }
    ...
    context "when the current user is not allowed to update the target user" do
      ...
      it "returns not found" do
        act(path: edit_admin_user_path(id: target_user.id))
        expect(response).to have_http_status(:not_found)
      end
    end
  ...
  describe "PATCH /admin/users/:id" do
    ...
    context "when the current user is allowed to update the target user" do
      ...
      it "calls UpdateUser and redirects to admin_users_path" do
        assert_redirect(to: admin_users_path) {
          act(path: admin_user_path(id: target_user.id), params: update_params)
        }
      end
    end
  ...
  describe "POST /admin/users/:id/send_login_link" do
    ...
    context "when the current user is allowed to send login links" do
      ...
      it "calls SendUserLoginLink and redirects to admin_users_path" do
        assert_redirect(to: admin_users_path) {
          act(path: send_login_link_admin_user_path(id: target_user.id))
        }
  ...
  def assert_form_rendered(submit_text: "Create ride requester")
    ...
    expect(response.body).to have_link("Cancel", href: admin_users_path)
