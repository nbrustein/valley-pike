...
RSpec.describe "OrganizationsMutate", type: :request do
  ...
  describe "GET /admin/organizations/new" do
    context "when the current user is allowed to create organizations" do
      ...
      it "renders the form" do
        assert_success { act(path: new_admin_organization_path) }
    ...
    context "when the current user is not allowed to create organizations" do
      ...
      it "returns not found" do
        act(path: new_admin_organization_path)
  ...
  describe "POST /admin/organizations" do
    context "when the current user is allowed to create organizations" do
      ...
      it "calls CreateOrganization and redirects to admin_organizations_path" do
        assert_redirect(to: admin_organizations_path) {
          act(path: admin_organizations_path, params: valid_create_params)
        }
      end
    end
  ...
  describe "GET /admin/organizations/:id/edit" do
    ...
    context "when the current user is allowed to update organizations" do
      it "renders the form" do
        assert_success { act(path: edit_admin_organization_path(id: organization.id)) }
    ...
    context "when the current user is not allowed to update organizations" do
      ...
      it "returns not found" do
        act(path: edit_admin_organization_path(id: organization.id))
        expect(response).to have_http_status(:not_found)
      end
    end
  ...
  describe "PATCH /admin/organizations/:id" do
    ...
    context "when the current user is allowed to update organizations" do
      ...
      it "calls UpdateOrganization and redirects to admin_organizations_path" do
        assert_redirect(to: admin_organizations_path) {
          act(path: admin_organization_path(id: organization.id), params: update_params)
        }
      end
    end
  ...
  describe "GET /admin/organizations/:id" do
    ...
    context "when signed out" do
      it "returns not found" do
        get(admin_organization_path(id: organization.id), headers:)
        expect(response).to have_http_status(:not_found)
      end
    end
    ...
    context "when the current user cannot view organizations" do
      ...
      it "returns not found" do
        act(path: admin_organization_path(id: organization.id))
        expect(response).to have_http_status(:not_found)
      end
    end
    ...
    context "when the organization does not exist" do
      it "returns not found" do
        act(path: admin_organization_path(id: 0))
        expect(response).to have_http_status(:not_found)
      end
    end
    ...
    context "when the current user can view organizations" do
      it "renders the organization name and abbreviation" do
        assert_success { act(path: admin_organization_path(id: organization.id)) }
      ...
      it "links back to the organizations index" do
        assert_success { act(path: admin_organization_path(id: organization.id)) }
        expect(response.body).to have_link("Back to organizations", href: admin_organizations_path)
      end
  ...
  def assert_form_rendered(submit_text: "Create organization")
    ...
    expect(response.body).to have_link("Cancel", href: admin_organizations_path)
