...
RSpec.describe "Ride request create form", type: :system do
  ...
  context "when filling in page 1" do
    ...
    def act
      visit new_admin_ride_request_path
    ...
    it "creates a draft and moves to page 2", :aggregate_failures do
      ...
      expect(page).to have_current_path(%r{/admin/ride_requests/[^/]+/edit/2})
  ...
  context "when filling in page 2" do
    ...
    def act
      visit admin_edit_ride_request_path(id: draft.id, page: 2)
  ...
  context "when filling in page 4" do
    ...
    def act
      visit admin_edit_ride_request_path(id: draft.id, page: 4)
    ...
    it "saves the contact info and moves to page 5", :aggregate_failures do
      act
      expect(page).to have_current_path(%r{/admin/ride_requests/[^/]+/edit/5})
      draft.reload
    ...
    it "defaults fields from the requester", :aggregate_failures do
      ...
      visit admin_edit_ride_request_path(id: no_contact_draft.id, page: 4)
  ...
  context "when publishing from page 5" do
    ...
    it "publishes the ride request and redirects to index", :aggregate_failures do
      visit admin_edit_ride_request_path(id: draft.id, page: 5)
      click_button "Publish"
      expect(page).to have_current_path(admin_ride_requests_path)
    ...
    it "shows errors when publishing fails" do
      ...
      visit admin_edit_ride_request_path(id: unpublishable.id, page: 5)
      click_button "Publish"
  ...
  context "when deleting a draft" do
    ...
    it "deletes the draft and redirects to index", :aggregate_failures do
      visit admin_edit_ride_request_path(id: draft.id, page: 1)
      accept_confirm do
    ...
    it "shows the delete link on every page" do
      (1..5).each do |pg|
        visit admin_edit_ride_request_path(id: draft.id, page: pg)
  ...
  context "when viewing a ride request as a vanita viewer" do
    ...
    it "displays the ride request details", :aggregate_failures do
      visit admin_ride_request_path(id: ride_request.id)
