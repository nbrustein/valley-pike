class AdminUsersMutateController < ApplicationController
  include Memery
  include ExecutesUnitsOfWork
  include AdminUserMutateConcerns::HasMutateUserForm
  helper AdminUsersHelper

  def show
    return render_not_found unless target_user.present?

    authorize(target_user, :show?, policy_class: AdminUserViewPolicy)
    render_form mode: :show, target_user:, submitted_params: nil
  end

  def new
    authorize(nil, :new?, policy_class: AdminUserMutatePolicy)
  ...
  def create
    success, errors = execute_unit_of_work(
      policy_meth: :create?,
      policy_class: AdminUserMutatePolicy
    ) do
    ...
    return redirect_to admin_users_path, notice: "User created." if success
  ...
  def edit
    return render_not_found unless target_user.present?
    authorize(target_user, :edit?, policy_class: AdminUserMutatePolicy)
  ...
  def update
    ...
    return redirect_to admin_users_path, notice: "User updated." if success
  ...
  def send_login_link
    ...
    if success
      redirect_to admin_users_path, notice: "Login link sent."
    else
  ...
  memoize def user_mutate_policy
    AdminUserMutatePolicy.new(current_user, nil)
  end
