class AdminOrganizationsMutateController < ApplicationController
  include Memery
  include ExecutesUnitsOfWork
  include AdminOrganizationMutateConcerns::HasMutateOrganizationForm

  def show
    return render_not_found unless target_organization.present?

    authorize(target_organization, :show?, policy_class: AdminOrganizationViewPolicy)
    render_form mode: :show, target_organization:, submitted_params: nil
  end

  def new
    authorize(nil, :new?, policy_class: AdminOrganizationMutatePolicy)
  ...
  def create
    success, _errors = execute_unit_of_work(
      policy_meth: :create?,
      policy_class: AdminOrganizationMutatePolicy
    ) do
    ...
    return redirect_to admin_organizations_path, notice: "Organization created." if success
  ...
  def edit
    ...
    authorize(target_organization, :edit?, policy_class: AdminOrganizationMutatePolicy)
  ...
  def update
    ...
    return redirect_to admin_organizations_path, notice: "Organization updated." if success
