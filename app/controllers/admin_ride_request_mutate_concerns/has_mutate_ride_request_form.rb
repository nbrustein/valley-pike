module AdminRideRequestMutateConcerns
  module HasMutateRideRequestForm
    extend ActiveSupport::Concern
    include Memery

    FORM_STEP_COMPONENTS = {
      1 => AdminRideRequestMutate::FormStep1Component,
      2 => AdminRideRequestMutate::FormStep2Component,
      3 => AdminRideRequestMutate::FormStep3Component,
      4 => AdminRideRequestMutate::FormStep4Component,
      5 => AdminRideRequestMutate::FormStep5Component,
    }.freeze
    ...
    def setup_instance_vars(mode:, ride_request:, page:)
      if mode == :create
        @form_action = if page == FORM_STEP_COUNT && ride_request&.id
          publish_admin_ride_request_path(id: ride_request.id)
        elsif ride_request&.id
          admin_edit_ride_request_path(id: ride_request.id, page:)
        else
          admin_ride_requests_path
        end
    ...
    memoize def permitted_organizations
      policy = AdminRideRequestMutatePolicy.new(current_user, nil)
    ...
    def destructive_action_for(ride_request)
      ...
      if ride_request.draft?
        {
          label: "Delete Draft",
          path: delete_draft_admin_ride_request_path(id: ride_request.id),
          method: :delete,
      ...
      elsif ride_request.cancellable?
        {
          ...
          path: cancel_admin_ride_request_path(id: ride_request.id),
          method: :post,
    ...
    def create_page_paths(ride_request)
      id = ride_request&.id
      [
        id ? admin_edit_ride_request_path(id:) : new_admin_ride_request_path,
        *(2..FORM_STEP_COUNT).map {|p| id ? admin_edit_ride_request_path(id:, page: p) : nil },
      ]
    end
  end
end
