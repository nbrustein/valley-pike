class AdminRideRequestsMutateController < ApplicationController
  include Memery
  include ExecutesUnitsOfWork
  include AdminRideRequestMutateConcerns::HasMutateRideRequestForm
  include CastsParams
  ...
  def show
    ...
    authorize(target_ride_request, :show?, policy_class: AdminRideRequestViewPolicy)
  ...
  def new
    authorize(nil, :new?, policy_class: AdminRideRequestMutatePolicy)
  ...
  def create
    ...
    success, _errors = execute_unit_of_work(
      policy_meth: :create?,
      policy_class: AdminRideRequestMutatePolicy
    ) do
    ...
    return redirect_to admin_edit_ride_request_path(id: uow.draft.id, page: 2) if success
  ...
  def edit
    ...
    authorize(target_ride_request, :edit?, policy_class: AdminRideRequestMutatePolicy)
  ...
  def update
    ...
    return redirect_to admin_edit_ride_request_path(id: target_ride_request.id, page: current_page + 1) if success
  ...
  def execute_ride_request_status_change(uow_class)
    ...
    return redirect_to admin_ride_requests_path if success
