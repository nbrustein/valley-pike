...
ActiveRecord::Schema[8.1].define(version: 2026_02_15_120000) do
  ...
  create_table "user_roles", id: :uuid, default: -> { "gen_random_uuid()" }, force: :cascade do |t|
    ...
    t.check_constraint "(role::text = ANY (ARRAY['developer'::character varying, 'vanita_admin'::character varying]::text[])) AND organization_id IS NULL OR (role::text = ANY (ARRAY['org_admin'::character varying, 'ride_requester'::character varying]::text[])) AND organization_id IS NOT NULL OR role::text = 'driver'::text", name: "user_roles_organization_id_check"
    t.check_constraint "role::text = ANY (ARRAY['developer'::character varying, 'vanita_admin'::character varying, 'org_admin'::character varying, 'ride_requester'::character varying, 'driver'::character varying]::text[])", name: "user_roles_role_check"
  end
